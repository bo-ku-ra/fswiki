...
sub do_action {
	...
	foreach(@list){
		if($self->{once}==0 ||($count>=$row && $count<$row+$self->{once})){
			$content = $content.
				"<li>".
				"<a href=\"".$wiki->create_page_url($_)."\">";
	#--------------------------------------------------------------------------------------------------------------------
	# bokura改
	#--------------------------------------------------------------------------------------------------------------------
			if($wiki->get_page_level($_) == 2){
				$content .="<span class=\"adminpage\">";
			}

			if($wiki->get_page_level($_) == 1){
				$content .="<span class=\"userpage\">";
			}

			$content .= Util::escapeHTML($_);

			if($wiki->get_page_level($_) == 2 || $wiki->get_page_level($_) == 1){
				$content .= "</span>";
			}
	#--------------------------------------------------------------------------------------------------------------------
			$content .=
